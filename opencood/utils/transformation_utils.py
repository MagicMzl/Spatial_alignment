...
def x_to_world_noise(pose, noise_std=0.0):
    """
    The transformation matrix from x-coordinate system to carla world system
    Also is the pose in world coordinate: T_world_x
    ...
    pose : list
        [x, y, z, roll, yaw, pitch], degree
    noise_std : float
        Standard deviation for the random noise to be added to the yaw angle
    ...
    x, y, z, roll, yaw, pitch = pose[:]
    #noise_stddev = random.choice([-noise_std, noise_std])
    if np.random.rand() < 0.5:  # 50% chance for each interval
        noise_stddev=np.random.uniform(-0.5, 0)
    else:
        noise_stddev=np.random.uniform(0, 0.5)
    #print('noise_stddev',noise_stddev)
    # Add random noise to the yaw angle
    #yaw_noise = np.random.normal(0, noise_std)
    roll += noise_stddev
...
def x1_to_x2_noise(x1, x2):
    ...
    x1_to_world = x_to_world_noise(x1, noise_std=0.5) # wP = x1_to_world * 1P, so x1_to_world is Tw1
    x2_to_world = x_to_world_noise(x2, noise_std=0.5) # Tw2
#     if sensor <0.5:
#         x1_to_world = x_to_world_noise(x1, noise_std=0.5) # wP = x1_to_world * 1P, so x1_to_world is Tw1
