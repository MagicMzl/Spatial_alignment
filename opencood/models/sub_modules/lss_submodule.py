...
class CamEncode_range_view(nn.Module):  # 提取图像特征进行图像编码
    def __init__(self, D, C, downsample, ddiscr, mode, use_gt_depth=False, depth_supervision=True):
        super(CamEncode_range_view, self).__init__()
        self.D = D  # 42
        self.C = C  # 64
        ...
        self.trunk = EfficientNet.from_pretrained("efficientnet-b0", in_channels = 5)  # 使用 efficientnet 提取特征
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            new_x : [B*N, C, D, fH, fW]
        """
        #print('x_img 5',x.shape)
        x_img = x[:,:,:,:]
        #print('x_img 6',x_img.shape)
...
class CamEncode(nn.Module):  # 提取图像特征进行图像编码
    def __init__(self, D, C, downsample, ddiscr, mode, use_gt_depth=False, depth_supervision=True):
        ...
        self.trunk = EfficientNet.from_pretrained("efficientnet-b0", in_channels = 3)  # 使用 efficientnet 提取特征
