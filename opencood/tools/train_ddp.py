...
def extract_module_keys(state_dict, module_prefix):
    new_state_dict = {key: value for key, value in state_dict.items() if key.startswith(module_prefix)}
    return new_state_dict
...
def main():
    ...
    #cam_saved_path = '/home/zonglin/MM_opv2v_three_space/opencood/logs/v2x_cam_range/net_epoch_bestval_at17.pth'
    print('#################### LOAD CAMERA NETWORK ####################')
    #model.load_state_dict(torch.load(
    #    cam_saved_path, map_location='cpu'), strict=False)
    cam_range_weights = torch.load(cam_saved_path)
    

    cam_prefix = 'camencode.'
    cam_model = extract_module_keys(cam_range_weights, cam_prefix)
    cam_model = remove_prefix(cam_model, cam_prefix)
    
    range_prefix = 'camencode_range_view.'
    cam_range_model = extract_module_keys(cam_range_weights, range_prefix)
    cam_range_model = remove_prefix(cam_range_model, range_prefix)
