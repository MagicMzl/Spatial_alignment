...
from opencood.utils.transformation_utils import x1_to_x2, x1_to_x2_noise
...
class OPV2VBaseDataset(Dataset):
    ...
    def decrease_string_by_one(self, s):
        # Convert the string to an integer, subtract 1, then format it back with leading zeros
        new_value = int(s) - 2
        if new_value < 0:
            return s
        return f"{new_value:0{len(s)}d}"
        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                for timestamp in timestamps:
                    ...
                    #lidar_timestamp = self.decrease_string_by_one(timestamp)
                    ...
                    if int(cav_id) < 0:
                        ...
                        lidar_file = os.path.join(cav_path,
                                              lidar_timestamp + '.pcd')
                        #lidar_file = os.path.join(cav_path,
                        #                      lidar_timestamp + '_transformed.pcd')
