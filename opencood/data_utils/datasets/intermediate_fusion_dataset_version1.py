...
from opencood.utils.transformation_utils import x1_to_x2, x_to_world, get_pairwise_transformation, get_camM_T_camXs
    ...
    class IntermediateFusionDataset(cls):
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            if self.load_camera_file:
                ...
                cam_to_vehicle_extrinsics = []
                #lidar_to_camera_list = []
                lidar2img_list = []
                for idx, img in enumerate(camera_data_list):
                    ...
                    lidar2img = viewpad @ lidar_to_camera
                    #print('camera_intrinsic',camera_intrinsic)
                    #print('lidar_to_camera',lidar_to_camera)
                    
                    '''
                    sensor2lidar_rotation = camera_to_lidar[:3,:3]
                    sensor2lidar_translation = camera_to_lidar[3,:3]
                    ...
                    #cam_to_vehicle_extrinsic = torch.from_numpy(cam_to_vehicle_extrinsic)
                    ...
                    #cam_to_vehicle_extrinsics.append(cam_to_vehicle_extrinsic)
                    #lidar_to_camera_list.append(torch.from_numpy(lidar_to_camera))
                    lidar2img_list.append(torch.from_numpy(lidar2img).float())
                    
                    #print('camera_to_lidar',camera_to_lidar)
                    #
                    #
                    #camera_to_lidar [[-9.8480773e-01 -2.7802138e-09 -1.7364861e-01  4.9999064e-01]
                    # [-1.7364861e-01  2.3854545e-08  9.8480773e-01  3.0000010e-01]
                    # [-1.4091542e-09 -1.0000000e+00  2.3973231e-08 -9.9999838e-02]
                    # [ 0.0000000e+00  0.0000000e+00  0.0000000e+00  1.0000000e+00]]
                    
                    rots.append(rot)
                    #print('rot',rot)
                    #[[ 5.7899e-07, -1.2585e-07, -1.0000e+00],
                    #[-1.0000e+00,  4.7465e-08, -5.8025e-07],
                    #[-4.7347e-08, -1.0000e+00,  1.1938e-07]]
                    ### add noise to camera extrin ###
                    '''
                    from scipy.spatial.transform import Rotation as R
                    rotation = R.from_matrix(rot)
                    euler_angles_xyz = rotation.as_euler('xyz', degrees=True)
                    #print('euler_angles_xyz',euler_angles_xyz)
                    #[-15.19234132 -89.99996295 -74.80761843]
                    noise = random.uniform(-5,)
                    euler_angles_xyz[0] = euler_angles_xyz[0] + 5
                    #print('euler_angles_xyz_noise',euler_angles_xyz)
                    #[-14.69234132 -89.99996295 -74.80761843]
                    r = R.from_euler('xyz', euler_angles_xyz, degrees=True)
                    rot = r.as_matrix()
                    #print('rotation_matrix',rotation_matrix)
                    #[[ 1.69452288e-07  9.99961917e-01 -8.72723784e-03]
                    # [-6.24015956e-07  8.72723784e-03  9.99961917e-01]
                    # [ 1.00000000e+00 -1.63999899e-07  6.25471042e-07]]
                    '''
                    ###################################
                    trans.append(tran)
                    #print('tran',tran)
                ...
                #print('singel vehicle',np.array(imgs).shape)
                selected_cav_processed.update(
                    {
                    "image_inputs": 
                        {
                            ...
                            #"cam_to_vehicle_extrinsics": torch.stack(cam_to_vehicle_extrinsics),
                            "rots": torch.stack(rots),
                            "trans": torch.stack(trans),
                            "post_rots": torch.stack(post_rots),
                            "post_trans": torch.stack(post_trans),
                            #"lidar_to_camera":lidar_to_camera_list,
        ...
        def __getitem__(self, idx):
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                #print('lidar_np',selected_cav_processed['lidar_np'].shape)
                origin_lidar_stack.append(selected_cav_processed['lidar_np'])
            ...
            #print('origin_lidar_stack',np.array(origin_lidar_stack).shape)
            processed_data_dict['ego'].update({'lidar_np': origin_lidar_stack})
        ...
        def collate_batch_train(self, batch):
            ...
            #print('lidar_np_list', np.array(lidar_np_list).shape)
            
            
            lidar_np_list = np.array(lidar_np_list)
            ...
            output_dict['ego'].update({'lidar_np': lidar_np_list})
